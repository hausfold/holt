...
| Worktree hooks | `~/.claude/settings.json` (yours, not the repo's) → `holt hook create` / `holt hook remove` | **This repo is what those hooks call**, so a broken `hook create` breaks every agent pane on the machine — including the one you're in. Exercise hook changes by piping JSON to the built binary, never by opening a pane. That file is *declared by the rice* (`modules/terminal`, nebelhaus#201) and re-asserted every rebuild, so it self-heals when Claude Code rewrites it — which also means hand-editing it to work around a bug here is reverted by the next `haus rebuild`. Fix it in holt. |
