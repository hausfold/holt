...
0. **Source it.** A paste is the usual case, and **the paste is the source of
   truth**: distill it, don't augment it from your own guesses. Nothing pasted
   means hand off THIS session — same shape, sourced from the conversation.
...
that this session is done with it.
...
The clipboard or the lane is how it actually travels; this print is so the user
can see where it starts and stops in a wall of transcript. Markers verbatim, on
their own lines:
...
use a longer one on the outside.

## Gaps

Too thin to ground — no repo, no path, no concrete objective? Produce the best
handoff the material supports, mark the holes inline as `UNKNOWN: <what>`, and
add one line after the closing marker: the single thing that would most improve
it. Don't interrogate the user for five fields, and don't refuse to produce
one.

**A brief with an UNKNOWN in its objective or its Next line does not get the
lane ending.** Take the clipboard, say which hole blocked it. Spawning an agent
onto a task nobody can start is how a lane burns a context window and lands
nothing.
