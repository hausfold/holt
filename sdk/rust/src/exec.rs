...
fn command(client: &HoltClient, args: &[&str]) -> Command {
    let mut cmd = Command::new(client.bin.as_deref().unwrap_or("holt"));
    cmd.args(args);
...
fn full_command(client: &HoltClient, args: &[&str]) -> Vec<String> {
    let mut command = vec![client.bin.as_deref().unwrap_or("holt").to_string()];
...
///
/// Returns [`HoltError`] on a non-zero exit, carrying holt's exit code
/// (SPEC.md §2.4) rather than collapsing every failure into one shape.
pub(crate) async fn run(client: &HoltClient, args: &[&str]) -> Result<RunResult, HoltError> {
    ...
    let output = cmd.output().await.map_err(|e| {
        ...
        HoltError::new(exit_code::USAGE, e.to_string(), full_command(client, args))
    })?;
    ...
    if !output.status.success() {
        ...
        return Err(HoltError::new(code, stderr, full_command(client, args)));
    }
    Ok(RunResult { stdout })
}
...
pub(crate) async fn run_json<T: DeserializeOwned>(
    ...
    serde_json::from_str(&result.stdout)
        .map_err(|e| HoltError::new(exit_code::USAGE, e.to_string(), full_command(client, args)))
}

/// Runs holt with stdio INHERITED from the calling process — used by
/// `new_interactive`/`resume_interactive`, where holt is expected to exec
/// an agent client and take over the real terminal.
pub(crate) async fn run_interactive(client: &HoltClient, args: &[&str]) -> Result<(), HoltError> {
    let mut cmd = command(client, args);
    ...
    if !status.success() {
        ...
        return Err(HoltError::new(code, "", full_command(client, args)));
    }
    Ok(())
}
