...
/// `scruff watch --json` as a stream of typed lines. One object per NDJSON
...
pub(crate) fn watch_all(
    client: ScruffClient,
) -> impl Stream<Item = Result<WatchLine, ScruffError>> + Send + 'static {
    stream! {
        let bin = client.bin.clone().unwrap_or_else(|| "scruff".to_string());
        ...
        let mut child = match cmd.spawn() {
            Ok(child) => child,
            Err(e) => {
                yield Err(ScruffError::new(exit_code::USAGE, e.to_string(), command_repr));
                return;
            }
        };
            ...
            match lines.next_line().await {
                Ok(Some(line)) => {
                    ...
                    match serde_json::from_str::<WatchLine>(trimmed) {
                        ...
                        Err(e) => {
                            yield Err(ScruffError::new(exit_code::USAGE, e.to_string(), command_repr.clone()));
                        }
                    }
                }
                Ok(None) => break,
        ...
        match child.wait().await {
            ...
            Ok(status) => {
                ...
                yield Err(ScruffError::new(code, stderr, command_repr));
            }
            Err(e) => {
                yield Err(ScruffError::new(exit_code::USAGE, e.to_string(), command_repr));
            }
